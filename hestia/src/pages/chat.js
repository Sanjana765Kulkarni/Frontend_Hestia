...
import { FaRegCopy, FaRegEdit, FaPlus, FaTrash, FaBars, FaTimes } from "react-icons/fa";
...
export default function Chat() {
  ...
  useEffect(() => {
    if (activeChatId) {
      ...
      setMessages([{ sender: "assistant", text: "Hello, I'm Hestia. How are you feeling today?" }]);
    }
  }, [activeChatId]);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages((prev) => [...prev.slice(0, -1), { sender: "user", text: input }, { sender: "assistant", text: reply }]);
      ...
      console.error(err);
      setMessages((prev) => [...prev.slice(0, -1), { sender: "assistant", text: "Sorry, failed to get response." }]);
    }
  ...
  const saveEdit = async (idx) => {
    ...
    if (messages[idx + 1] && messages[idx + 1].sender === "assistant") {
    ...
    try {
      ...
      setMessages((prev) => {
        ...
        newMessages.splice(idx + 1, 0, { sender: "assistant", text: reply });
        return newMessages;
      });
    } catch (err) {
      console.error(err);
      setMessages((prev) => {
        const newMessages = [...prev];
        newMessages.splice(idx + 1, 0, { sender: "assistant", text: "Sorry, failed to get response." });
        return newMessages;
      });
    }
    setLoading(false);
  ...
  return (
    <div className="relative flex h-screen bg-[#181611] text-white" style={{ fontFamily: 'Manrope, Noto Sans, sans-serif' }}>
      {/* Header */}
      <header className="absolute top-0 left-0 right-0 h-16 bg-black flex items-center justify-between px-4 border-b border-solid border-b-[#483e23] z-50">
        <div className="flex items-center gap-4">
          {!isSidebarOpen && (
            <button onClick={() => setIsSidebarOpen(true)} className="text-white hover:text-[#f4c653]">
              <FaBars size={20} />
            </button>
          )}
      ...
      <aside className={`flex-shrink-0 bg-[#100e0b] transition-all duration-300 ease-in-out ${isSidebarOpen ? 'w-64' : 'w-0'} overflow-hidden`}>
        <div className="h-full flex flex-col pt-20 p-4">
          ...
          <button onClick={handleNewChat} className="flex items-center gap-2 w-full p-2 rounded-lg text-white hover:bg-[#221d11] mb-4">
            <FaPlus /> New Chat
          </button>
          <div className="space-y-2 overflow-y-auto">
            {chats.map(chat => (
              <div key={chat.id} className={`flex items-center justify-between p-2 rounded-lg cursor-pointer font-sans ${activeChatId === chat.id ? 'bg-[#f4c653] text-black' : 'text-[#f4c653] hover:bg-[#221d11]'}`}>
      ...
      <main className="flex-1 flex flex-col">
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-10 pt-20 pb-24">
        ...
        <div className={`fixed bottom-0 right-0 transition-all duration-300 ease-in-out ${isSidebarOpen ? 'left-64' : 'left-0'}`}>
          <div className="bg-[#181611] border-t border-[#221d11] p-4">
